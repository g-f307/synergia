...
export class PendingDetailComponent implements OnInit {
  ...
  private run(operation:ReturnType<PendingService['submitApproval']>):void{if(this.actionBusy())return;this.actionBusy.set(true);this.actionError.set('');operation.subscribe({next:value=>{this.actionBusy.set(false);this.approval.set(value)},error:()=>{this.actionBusy.set(false);this.actionError.set(this.i18n.t('approval.actionError'))}})}
