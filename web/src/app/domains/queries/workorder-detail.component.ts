...
import { isStale, isWorkorderPartial } from './query-response-state';
  ...
  template: `
    <section class="workorder-detail" aria-labelledby="workorder-title">
      ...
      @if (result(); as current) {
        ...
        <section class="card"><h2>{{ i18n.t('queries.pending') }}</h2>@if(!current.pending_items.length){<p>{{ i18n.t('queries.none') }}</p>}@for(item of current.pending_items;track item.id){<article><div><strong>{{ item.category }}</strong><span class="badge">{{ item.status }}</span></div><p>{{ item.reason || i18n.t('common.notAvailable') }}</p><small>{{ item.priority }} · {{ i18n.formatDate(item.updated_at,{dateStyle:'short',timeStyle:'short'}) }}</small></article>}</section>
...
export class WorkorderDetailComponent implements OnInit {
  ...
  isPartial(result: ConsolidatedWorkorder): boolean { return isWorkorderPartial(result); }
  isOutdated(result: ConsolidatedWorkorder): boolean { return isStale(result.workorder.updated_at); }
