...
describe('ExecutionService', () => {
  ...
  it('lists executions with stable paging and combined filters',()=>{service.list({organizationId:'org-1',status:'completed',lifecycle:'completed',source:'OWM',fileType:'csv',dateFrom:'2026-09-01T00:00',dateTo:'2026-09-02T00:00',executionId:'exec-1',page:2,pageSize:20,sort:'newest'}).subscribe();const request=http.expectOne(value=>value.url.endsWith('/executions'));expect(request.request.params.get('organization_id')).toBe('org-1');expect(request.request.params.get('lifecycle')).toBe('completed');expect(request.request.params.get('execution_id')).toBe('exec-1');expect(request.request.params.get('page')).toBe('2');expect(request.request.params.get('sort')).toBe('newest');request.flush({items:[],pagination:{page:2,page_size:20,total:0,pages:0},sort:'newest'});});
