...
import { ConfirmationComponent, PaginationComponent } from '../../shared/ui/ui-kit';
import { Classification, Divergence, Evidence, Execution, ExecutionTab, Page, PendingItem } from './execution.models';
...
@Component({ imports: [RouterLink, NgTemplateOutlet, ConfirmationComponent, PaginationComponent], template: `
...
@if(loading()){<p role="status">{{ i18n.t('executions.loading') }}</p>} @if(failure()){<div class="card error" role="alert"><h2>{{ i18n.t('executions.loadError') }}</h2><p>{{ failure()?.message }}</p></div>}
@if(item(); as current){
  ...
  @if(tab()==='summary'){<section class="card"><h2>{{ i18n.t('executions.counts') }}</h2><div class="metrics">@for(entry of countEntries();track entry[0]){<div><strong>{{ i18n.formatNumber(entry[1]) }}</strong><span>{{ entry[0] }}</span></div>}</div></section>}
...
export class ExecutionDetailComponent implements OnInit {
 ...
 readonly tabs=TABS; readonly item=signal<Execution|null>(null); readonly loading=signal(true); readonly failure=signal<ApiFailure|null>(null); readonly tab=signal<ExecutionTab>('summary'); readonly pageData=signal<Page<Divergence|Classification|PendingItem|Evidence>|null>(null); readonly sectionLoading=signal(false); readonly sectionFailure=signal<ApiFailure|null>(null); readonly severity=signal(''); readonly source=signal(''); readonly confirming=signal(false); readonly reprocessedId=signal(''); private id='';
 readonly countEntries=computed(()=>Object.entries(this.item()?.counts??{}));
 ...
 reprocess():void{this.confirming.set(false);this.api.reprocess(this.id,'web-execution-monitor').subscribe({next:value=>this.reprocessedId.set(value.execution_id),error:(e:ApiFailure)=>this.sectionFailure.set(e)})}
}
