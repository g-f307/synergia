...
describe('ReportCatalogComponent', () => {
  let fixture: ComponentFixture<ReportCatalogComponent>; let response: Subject<ReportPage>; let params: BehaviorSubject<ReturnType<typeof convertToParamMap>>; let router: jasmine.SpyObj<Router>; let create: jasmine.Spy; let list: jasmine.Spy; const granted = signal(['report.read', 'report.generate', 'report.export']);
  beforeEach(async () => {
    ...
    create = jasmine.createSpy().and.returnValue(of(reportDetail())); list = jasmine.createSpy().and.returnValue(response.asObservable());
    await TestBed.configureTestingModule({ imports: [ReportCatalogComponent], providers: [provideRouter([]), { provide: Router, useValue: router }, { provide: ActivatedRoute, useValue: { queryParamMap: params.asObservable() } }, { provide: ReportService, useValue: { policy: () => of(reportPolicy()), list, create, export: () => of({ body: new Blob(['safe']), headers: { get: () => null } }) } }, { provide: SessionService, useValue: { hasPermission: (key: string) => granted().includes(key) } }] }).compileComponents();
