...
| Consolidar por Workorder, Demand ID, lote, modelo e serial | F | precedência de quantidades; proveniência; conflitos não contaminam Workorder | [#8](https://github.com/g-f307/synergia/issues/8) | `backend/app/consolidation.py` | `test_consolidation.py`; `consolidation_records.json`; `wo-status-reference.csv` | Atendido |
| Classificar OQC, holds, rework, bloqueio, aging e impacto em container | F | catálogo versionado; categorias simultâneas; fila ativa determinística | [#9](https://github.com/g-f307/synergia/issues/9) | `backend/app/business_rules.py`, `model/business_rules.json` | `test_business_rules.py`; `rules_scenarios.json` | Atendido |
