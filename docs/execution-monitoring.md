...
- `GET /executions/{id}/divergences`: erros e avisos paginados. Aceita `source`,
  ...
  `sort=oldest|newest`.
- `GET /executions/{id}/classifications`: classificações e respectivas regras,
  prioridades, justificativas e evidências estruturadas.
- `GET /executions/{id}/pending-items`: pendências da execução.
- `GET /executions/{id}/evidences`: catálogo seguro dos arquivos aprovados.
- `GET /executions/{id}/evidences/{evidence_id}/download`: download controlado.

A ordenação de divergências é estável por data e identificador. As evidências são
...
Com a API iniciada e uma execução criada pelas massas sintéticas:

```bash
API_URL=http://localhost:8000 EXECUTION_ID=<id> scripts/demo_execution_monitoring.sh
```

O roteiro consulta visão geral, divergências, classificações, pendências e evidências,
sem exigir rede corporativa ou RPA.
